...
import SessionsBookingLayout from "../../Layouts/SessionsBookingLayout";
...
export default function BookingsSessions() {
  ...
  const [dialogType, setDialogType] = useState(""); // "booking" or "session"
  ...
  const [isSSessionsBookingLayoutOpen, setSessionsBookingLayoutOpen] = useState(false);
  // Attendance
  ...
  const fetchAllData = async () => {
    try {
      ...
      const loadedBookings = bookingRes.data.bookings.map((b) => ({
        ...b,
        BookingTime: dayjs(b.BookingTime, "HH:mm:ss").isValid() ? dayjs(b.BookingTime, "HH:mm:ss").format("HH:mm:ss") : "00:00:00",
      }));
      

      // 2) Sessions
      const sessionRes = await axios.get("/booking/sessions");
      const loadedSessions = sessionRes.data.sessions.map((s) => ({
        ...s,
        StartTime: dayjs(s.StartTime, "HH:mm:ss").isValid() ? dayjs(s.StartTime, "HH:mm:ss").format("HH:mm:ss") : "00:00:00",
        EndTime: dayjs(s.EndTime, "HH:mm:ss").isValid() ? dayjs(s.EndTime, "HH:mm:ss").format("HH:mm:ss") : "00:00:00",
      }));
    ...
    const formatDateTime = (dateString) => {
      ...
      const dateObj = dayjs(dateString, ["YYYY-MM-DD HH:mm:ss", "YYYY-MM-DDTHH:mm:ssZ"], true);
    
      return dateObj.isValid() ? dateObj.format("MMMM D, YYYY h:mm A") : "Invalid Date/Time";
    };
  ...
  const bookingColumns = [
    ...
    { field: "BookingDate", headerName: "Date", width: 180,  renderCell: (params) => params.value ? formatDate(params.value) : "—" },
    { field: "BookingTime", headerName: "Time", width: 80,  renderCell: (params) => params.value ? formatTime(params.value) : "—"},
  ...
  const sessionColumns = [
    ...
    { field: "StartTime", headerName: "Start", width: 120, renderCell: (params) => params.value ? formatTime(params.value) : "—"},
    { field: "EndTime", headerName: "End", width: 120, renderCell: (params) => params.value ? formatTime(params.value) : "—" },
    ...
    renderCell: (params) => (
      <Box sx={{ display: "flex", gap: 1 }}>
        ...
        <Tooltip title="Mark Attendance"  sx={{ color: "black"}}>
          <Button
  ...
  const handleExportPDF = () => {
    ...
    if (filteredBookings.length === 0 && filteredSessions.length === 0 && coaches.length === 0) {
    ...
    const coverPage = "/imgs/coverpage2.png";  // ✅ Ensure correct path
    const addPage = "/imgs/addpage2.png";      // ✅ Ensure correct path
    ...
    if (activeTab === 0) {
      ...
      tableHeaders = ["ID", "Branch", "Member", "Facility", "Date", "Time", "Hrs", "Status"];
      ...
      tableHeaders = ["ID", "Session Name", "Coach", "Start", "End", "Cap", "Joined", "Status"];
      ...
      tableHeaders = ["ID", "Name", "Specialty", "Availability", "Contact"];
    ...
    const getStatusColor = (status) => {
      if (status?.toLowerCase() === "completed" || status?.toLowerCase() === "paid") {
        return "#4CAF50";  // Green for Completed/Paid
      }
      return "#333333"; // Default black for other statuses
    };
  
    // ✅ Generate Table with Status Colors
    doc.autoTable({
      head: [tableHeaders],
      body: tableBody,
      startY: startY,
      ...
      didParseCell: (data) => {
        if (activeTab === 0 || activeTab === 1) { // ✅ Only for Bookings/Sessions
    ...
    const pdfFilename =
      activeTab === 0 ? "BookingsReport.pdf" :
      activeTab === 1 ? "SessionsReport.pdf" :
      "CoachesReport.pdf";
                ...
                <List dense sx={{ maxHeight: 650, overflowY: "auto" }}>
                  {currentEvents.map((ev) => (
                    ...
                    >
                        ...
                        secondary={`Date: ${formatDate(ev.date)}`}
                      />
        ...
        <Paper sx={{ p: 2, mb: 3 }}>
          ...
          >
            ...
            {activeTab === 1 && (
              <>
                {/* Manage Attendance */}
                <Button variant="outlined" onClick={() => setSessionsBookingLayoutOpen(true)}>
                 View Attendance & Waitlist
                </Button>
 ...
 {/* Manage Plans Layout */}
      {isSSessionsBookingLayoutOpen && <SessionsBookingLayout onClose={() => setSessionsBookingLayoutOpen(false)} />}
          {/* DATAGRID */}
        ...
        {/* ADD Booking Dialog */}
        <Dialog open={isAddBookingOpen} onClose={() => setAddBookingOpen(false)} fullWidth maxWidth="lg">
          {/* DIALOG TITLE */}
          <DialogTitle>
            <Box display="flex" justifyContent="space-between" alignItems="center">
              ...
              <IconButton onClick={() => setAddBookingOpen(false)} sx={{
                  color: "inherit",
                  "&:hover": { color: "red" },
                }}>
                <CloseIcon />
            ...
            <Box sx={{ p: 2 }}>
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                <Grid container spacing={2}>
                  ...
                  {/* Booking Date & Booking Time (Side by Side) */}
                  <Grid item xs={12} sm={6}>
                ...
                <Box sx={{ mt: 4, display: "flex", flexDirection: "row", gap: 3, justifyContent: "flex-end" }}>
                  <Button variant="contained" color="primary" onClick={() => {setDialogType("booking"); setOpenConfirmation(true);}}>
                    <SaveIcon /> Save Booking
                  </Button>
                </Box>
              </form>
            </Box>
        ...
        <DialogContent dividers sx={{ p: 4 }}>
          {selectedBooking && (
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <TimePicker
                label="Booking Time"
                value={selectedBooking.BookingTime ? dayjs(selectedBooking.BookingTime, "HH:mm") : null}
                onChange={(newVal) => {
                  const formattedTime = newVal ? newVal.format("HH:mm") : "";
                  setSelectedBooking({ ...selectedBooking, BookingTime: formattedTime });
                }}
                renderInput={(params) => <TextField {...params} size="small" fullWidth />}
              />
      ...
      {/* ADD Session Dialog */}
      <Dialog open={isAddSessionOpen} onClose={() => setAddSessionOpen(false)} fullWidth maxWidth="lg">
        {/* DIALOG TITLE */}
        <DialogTitle>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h5">
              <FitnessCenterIcon sx={{verticalAlign: "middle", mr: 1}} />
              Add New Session</Typography>
            <IconButton onClick={() => setAddSessionOpen(false)} sx={{
                color: "inherit",
                "&:hover": { color: "red" },
              }}>
              <CloseIcon />
        ...
        <DialogContent dividers>
          <Box sx={{ p: 2 }}>
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", gap: 2 }}>
                    ...
                    <TimePicker
                      label="Start Time"
                      value={newSession.StartTime && dayjs(newSession.StartTime, "HH:mm:ss").isValid()
                        ? dayjs(newSession.StartTime, "HH:mm:ss")
                        : null
                      }
                      onChange={(timeVal) => {
                        setNewSession((prev) => ({ ...prev, StartTime: timeVal ? timeVal.format("HH:mm:ss") : "00:00:00" }));
                      }}
                      renderInput={(params) => <TextField {...params} fullWidth required />}
                    />
                    </FormControl>
                ...
                <Grid item xs={12} sm={6}>
                  <Box sx={{ display: "flex", gap: 2 }}>
                    ...
                    <TimePicker
                      label="End Time"
                      value={newSession.EndTime && dayjs(newSession.EndTime, "HH:mm:ss").isValid()
                        ? dayjs(newSession.EndTime, "HH:mm:ss")
                        : null
                      }
                      onChange={(timeVal) => {
                        setNewSession((prev) => ({ ...prev, EndTime: timeVal ? timeVal.format("HH:mm:ss") : "00:00:00" }));
                      }}
                      renderInput={(params) => <TextField {...params} fullWidth required />}
                    />
                    </FormControl>
              ...
              {/* BUTTONS */}
              <Box sx={{ mt: 4, display: "flex", flexDirection: "row", gap: 3, justifyContent: "flex-end" }}>
                <Button variant="contained" color="primary" onClick={() => {setDialogType("session"); setOpenConfirmation(true)}}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <TextField
                  ...
                  variant="filled"
                  InputProps={{ readOnly: true }}
                  value={formatTime(selectedSession.StartTime || "—")}
                  sx={{ mb: 2 }}
                />
              </Grid>
              <Grid item xs={6}>
                <TextField
                  ...
                  variant="filled"
                  InputProps={{ readOnly: true }}
                  value={formatTime(selectedSession.EndTime || "—")}
                  sx={{ mb: 2 }}
                />
              </Grid>
              <Grid item xs={6}>
      ...
      >
        ...
        >
          ...
          {/* Dialog Actions - Book aligned to the right */}
          <DialogActions sx={{ justifyContent: "flex-end", py: 2 }}>
            <Button
              variant="contained"
              color="primary"
      ...
      {/* ADD Coach Dialog */}
        <Dialog open={isAddCoachOpen} onClose={() => setAddCoachOpen(false)} fullWidth maxWidth="lg">
          {/* DIALOG TITLE */}
          <DialogTitle>
            <Box display="flex" justifyContent="space-between" alignItems="center">
              <Typography variant="h5">
                <PersonIcon sx={{verticalAlign: "middle", mr:1}}/>
                Add New Coach</Typography>
              <IconButton onClick={() => setAddCoachOpen(false)} sx={{
                  color: "inherit",
                  "&:hover": { color: "red" },
                }}>
                <CloseIcon />
          ...
          <DialogContent dividers>
            <Box sx={{ p: 2 }}>
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                ...
                {/* BUTTONS */}
                <Box sx={{ mt: 4, display: "flex", flexDirection: "row", gap: 3, justifyContent: "flex-end" }}>
                  <Button variant="contained" color="primary" onClick={() => {setDialogType("coach"); setOpenConfirmation(true); }} >
                    <SaveIcon  />  Save Coach
                  </Button>
                </Box>
              </form>
